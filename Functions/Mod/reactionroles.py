...
class Ext(Extension):
    ...
    @extension_command(name="rrsetup",
    ...
    async def _rrsetup(self, ctx: CommandContext, channel):
        msg = 'Click on the <:forgeapprove:980531953801494598> **Notifications** button to get notified when an announcement is posted\nClick on the <:forgethreaten:980531953667309608> **Spoilers** button to get notified when a spoiler/teaser is posted\nClick on any of the buttons again to disable notifications'

        btn_notifications = Button(style=ButtonStyle.PRIMARY,
                                   label="Notifications",
                                   custom_id="btn_notifications")

        btn_spoilers = Button(style=ButtonStyle.PRIMARY,
                              label="Spoilers",
                              custom_id="btn_spoilers")

        btn_notifications.emoji = await ctx.guild.get_emoji(934186475736678480)
        ...
        await channel.send(msg, components=[btn_notifications, btn_spoilers])
    ...
    async def reaction_role(self, ctx, role_id):
        ...
        if role_id in usr.roles:
            ...
            await ctx.send(f"You no longer have the <@&{role_id}> role!",
                           ephemeral=True)
        else:
            await usr.add_role(role_id, ctx.guild_id)
            await ctx.send(f"You now have the <@&{role_id}> role!",
                           ephemeral=True)

    @extension_component("btn_notifications")
    async def btn_notifications_resp(self, ctx):
        await self.reaction_role(ctx, NOTIFICATIONS_ROLE_ID)
